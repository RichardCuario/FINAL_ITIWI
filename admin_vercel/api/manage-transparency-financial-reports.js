export default async function handler(req, res) {
  ...
  const tableName = 'transparency_financial_reports';
    ...
    try {
      ...
      if (!response.ok) {
        return res.status(response.status).json({
          error: 'Failed to load financial report records.',
      ...
      return res.status(500).json({
        error: 'Unexpected error while loading financial report records.',
  ...
  try {
    if (method === 'POST') {
      ...
      return await handleSupabaseResponse(
        response,
        res,
        'Failed to create financial report record.',
      );
    }
    ...
    if (method === 'PATCH') {
      ...
      return await handleSupabaseResponse(
        response,
        res,
        'Failed to update financial report record.',
      );
    }
    ...
    return await handleSupabaseResponse(
      response,
      res,
      'Failed to delete financial report record.',
    );
  } catch (error) {
    return res.status(500).json({
      error: 'Unexpected error while managing financial report record.',
      details: error instanceof Error ? error.message : String(error),
    });
  }
}
