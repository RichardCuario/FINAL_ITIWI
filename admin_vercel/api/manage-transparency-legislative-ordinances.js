export default async function handler(req, res) {
  ...
  const tableName = 'transparency_legislative_ordinances';
  ...
  const transparencyId = String(body?.id ?? body?.transparencyId ?? body?.recordId ?? '').trim();
  const payload = sanitizeTransparencyPayload(body?.payload ?? body);

  if (method === 'GET') {
    try {
      const response = await fetch(`${supabaseUrl}/rest/v1/${tableName}?select=id,title,description,pdf_url,is_published,display_order,created_at,updated_at&order=display_order.asc.nullslast,created_at.desc`, {
        method: 'GET',
        headers: {
  ...
  try {
    if (method === 'POST') {
      ...
      return await handleSupabaseResponse(response, res, 'Failed to create transparency record.');
    }

    if (method === 'PATCH') {
      const response = await fetch(`${supabaseUrl}/rest/v1/${tableName}?id=eq.${encodeURIComponent(transparencyId)}`, {
        method: 'PATCH',
        headers: {
          apikey: serviceRoleKey,
          Authorization: `Bearer ${serviceRoleKey}`,
      ...
      return await handleSupabaseResponse(response, res, 'Failed to update transparency record.');
    }
    ...
    return await handleSupabaseResponse(response, res, 'Failed to delete transparency record.');
  } catch (error) {
    return res.status(500).json({
